...
一个 Xposed 模块，做两件互不相关的事 —— 都是「系统给应用图标套了你不想要的形状」的毛病：

| 功能 | 解决什么 | 默认 |
|---|---|---|
| **强制圆形图标** | 桌面 / 应用列表 / 最近任务里的应用图标形状由 ROM 决定（圆角方、水滴、方），不一定是圆。这一路把它**一律裁成正圆** | 开 |
| **修复通知小图标** | Android 12 起系统会强行把通知小图标统一着色，没做单色适配的应用会显示成一坨认不出是谁的灰白色块。这一路把它**换回能认出是谁的样子** | 开（系统黑白） |

两个功能各有一个总开关，互不影响；改完**不需要重启**，新加载的图标按新设置走。
...
三层 adaptive icon（背景 / 前景 / 单色层），字符是 **Oi**：极淡粉背景 `#FCE4EC`
+ 大号黑字，monochrome 只留白色剪影，对齐 Mihon 的主题图标结构。
